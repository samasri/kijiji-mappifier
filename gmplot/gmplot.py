...
class GoogleMapPlotter(object):
    ...
    def geocode(self, location_string):
        geocode = requests.get(
            'https://maps.googleapis.com/maps/api/geocode/json?address="%s&key=%s' % (location_string, self.apikey))
    ...
    def marker(self, lat, lng, color='#FF0000', c=None, title="no implementation", website='https://www.google.com'):
        if c:
            color = c
        color = self.color_dict.get(color, color)
        color = self.html_color_codes.get(color, color)
        self.points.append((lat, lng, color[1:], title, website))
    ...
    def write_point(self, f, lat, lon, color, title, link):
        ...
        f.write('\t\tmarker.addListener("click", function() { document.location.href = "%s"; });\n' % link)
        f.write('\n')
